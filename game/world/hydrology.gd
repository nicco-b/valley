...
## STRATA_CONTOUR=1) routes the same Hydrology system through the PERSISTENT HELD
...
func _contour_resolve() -> void:
	...
	if err != "":
		...
		_contour_mode = -1
		return
	# Hydrology is a SINGLETON held-world system (its held world holds the region
	# reservoir storage alone), so the held path drops the pure-persistent-write
	...
	# default; on, the region-river tick routes through the persistent held world.
...
## rebuilds the held world from the restored save (the next tick re-creates it),
