...
// Both channels ride: R height, G foam memory (S2).
layout(local_size_x = 8, local_size_y = 8, local_size_z = 1) in;
layout(rg16f, set = 0, binding = 0) uniform restrict readonly image2D src;
layout(rg16f, set = 0, binding = 1) uniform restrict writeonly image2D dst;
...
void main() {
	...
	if (q.x >= 0 && q.y >= 0 && q.x < pc.grid && q.y < pc.grid) {
		v = imageLoad(src, q).rg;
	}
	imageStore(dst, p, vec4(v, 0.0, 0.0));
}
